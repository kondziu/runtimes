...
impl Program {
    pub fn new(constants: ConstantPool, slots: GlobalSlots, entry: ConstantPoolIndex) -> Program {
        Program {constants, slots, entry}
    }
}

impl Serializable for Program {
    fn serialize<W: Write>(&self, sink: &mut W) -> () {
        self.constants.serialize(sink);
        self.slots.serialize(sink);
    ...
    fn from_bytes<R: Read>(input: &mut R) -> Self {
        ...
        Program {
            constants: ConstantPool::from_bytes(input),
            slots: GlobalSlots::from_bytes(input),
